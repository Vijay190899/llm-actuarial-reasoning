...
\end{itemize}
The method borrows the idea of procedurally generated, code-verifiable reasoning problems from recent work on financial chains of thought (arXiv:2506.02515), and extends it to actuarial life contingencies, to first-error localization, to rewording stability, and to prompt mitigation.

\begin{figure}[t]\centering
\begin{tikzpicture}[>=Latex, node distance=8mm and 9mm,
  box/.style={draw, rounded corners, align=center, inner sep=4pt, font=\small, minimum height=11mm}]
\node[box] (engine) {Validated engine\\Makeham SULT,\\interest and life};
\node[box, right=of engine] (gen) {Procedural generator\\problem, gold anchor\\steps, 2 rewordings};
\node[box, right=of gen] (model) {Language model\\show work,\\end with ANSWER};
\node[box, right=of model] (parse) {Extract numbers\\and final answer};
\node[box, below=14mm of gen, minimum width=52mm] (grade)
   {Grade final within 1\%\\Localize first wrong anchor\\Compare the 3 rewordings};
\draw[->] (engine) -- (gen);
\draw[->] (gen) -- (model);
\draw[->] (model) -- (parse);
\draw[->] (parse) |- (grade);
\draw[->] (gen) -- (grade) node[midway, left, font=\scriptsize] {gold};
\end{tikzpicture}
